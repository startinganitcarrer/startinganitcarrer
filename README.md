
# 👋 Hi, I'm Sandip Dhungana  
### IT Helpdesk Support | IT Technician | Active Directory | Azure | Office 365 | VMware

I'm an aspiring IT Support Specialist based in Canada, passionate about troubleshooting, system administration, and building real-world IT labs.  

I enjoy creating hands-on projects that simulate enterprise environments from Active Directory to osTicket help desk systems.

---

## 🧰 IT Projects

### 🔹 [Active Directory Home Lab](https://github.com/startinganitcarrer/Active-Directory-Lab)
A full Windows Server 2022 domain environment with DHCP, DNS, Group Policy, and user management.

### 🔹 [OS Ticket Help Desk System](https://github.com/startinganitcarrer/OS-Ticket)
A complete help desk ticketing system with SLAs, automation, departments, and ticket workflows.

### 🔹 [Network Troubleshooting with Azure NSGs](https://github.com/startinganitcarrer/Networking-Troubleshooting)
Hands-on Azure networking lab focused on NSG rules, connectivity testing, and troubleshooting.

---

## 🎓 Certifications
- [CompTIA A+](https://www.credly.com/badges/0b7514fb-684d-4196-adf6-7886372c78fc/linked_in_profile)
- Zendesk Professional Customer Service

---

## 🛠️ Technical Skills
- **Operating Systems:** Windows Server, Windows 10/11, Linux  
- **Networking:** DNS, DHCP, VPN, Firewalls, Azure NSGs  
- **Tools:** osTicket, Active Directory, Group Policy, Office 365  
- **Virtualization:** VMware, Hyper‑V  
- **Troubleshooting:** Hardware, software, connectivity, user support  

---

## 🤳 Connect With Me

[<img align="left" alt="LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]
[<img align="left" alt="Instagram" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/instagram.svg" />][instagram]

<br/>

[linkedin]: https://www.linkedin.com/in/sandipdhungana
[instagram]: https://www.instagram.com/_.sandippppp/




